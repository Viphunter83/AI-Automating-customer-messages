...
async def health_check_openai():
    ...
    try:
        ...
        test_result = await asyncio.wait_for(
            ai_classifier.classify("test", client_id="health_check"),
            timeout=5.0
        )
...
async def health_check_webhook():
    ...
    try:
        ...
        return {
            "status": "healthy",
            "webhook": "configured",
            "url": webhook_url.split('@')[-1] if '@' in webhook_url else webhook_url[:50] + "..." if len(webhook_url) > 50 else webhook_url
        }
    except Exception as e:
...
async def health_check_full():
    ...
    try:
        webhook_sender = WebhookSender()
        if webhook_sender.platform_webhook_url and webhook_sender.platform_webhook_url != "http://localhost:9000/webhook/response":
