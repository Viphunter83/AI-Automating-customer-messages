...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def _is_suspicious_request(self, request: Request) -> bool:
        ...
        if not is_safe_path:
            if self._check_patterns(path, self.SQL_INJECTION_PATTERNS + self.PATH_TRAVERSAL_PATTERNS):
                return True
        ...
        if query_string:
            ...
            if '/api/search' in path and 'query=' in query_string:
                ...
                if self._check_patterns(query_string, self.SQL_INJECTION_PATTERNS + self.XSS_PATTERNS):
                    return True
        ...
        for header_name, header_value in request.headers.items():
            ...
            if header_name.lower() in ['host', 'accept', 'content-type', 'authorization']:
                continue
            if self._check_patterns(str(header_value), self.SQL_INJECTION_PATTERNS + self.XSS_PATTERNS):
                return True
        ...
        for pattern in patterns:
            try:
                if re.search(pattern, text_lower):
                    logger.debug(f"Pattern matched: {pattern[:50]} in text: {text[:100]}")
                    return True
            except re.error:
